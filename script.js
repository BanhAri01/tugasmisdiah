...
document.getElementById('send-final').onclick = (e) => {
  ...
  // susun objek data sesuai variabel di template EmailJS
  const data = {
    user_name:       userName,         // di template: {{user_name}}
    user_whatsapp:   userWA,           // di template: {{user_whatsapp}}
    project_type:    projectType,      // di template: {{project_type}}
    project_details: projectDetails,   // di template: {{project_details}}
    project_deadline: projectDeadline, // di template: {{project_deadline}}
    // (opsional) kirim timestamp server juga
